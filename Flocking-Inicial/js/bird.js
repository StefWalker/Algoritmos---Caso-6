var Bird = function(scene, octree, obstacles, velocity_hash, position, orientation, pID) {
	...
	this.distance = 0;		// Distancia generada del mismo
}
...
Bird.prototype.update_position = function(elapsed_time, lookAt) {
	...
	if (elapsed_time/1000 < .5) {
		...
		// Console log encargado de imprimir los nuevos datos generados, con orden " ID Distancia Velocidad " (no acomulados)
		console.log( this.id + " " + Math.sqrt( Math.pow(displacement.x , 2) + Math.pow(displacement.y , 2) + Math.pow(displacement.z , 2) ) + " " + (this.velocity.x + this.velocity.y + this.velocity.z)/3 + " ");
	}
