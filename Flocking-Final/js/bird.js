var Bird = function(scene, octree, obstacles, velocity_hash, position, orientation, pID) {
	this.scene = scene;
	this.octree = octree;
	...
	this.ID = pID;					// ID del Bird
	this.DistanciaAcumulada = 0;	// Distancia acumulada
	this.VelocidadAcumulada = 0;	// Velocidad acumulada
}

Bird.prototype.init_mesh = function(position, orientation) {
	var geometry = new BirdObject();
	var material =  new THREE.MeshBasicMaterial( { color: 0x828282, side: THREE.DoubleSide } )
...
Bird.prototype.update_position = function(elapsed_time, lookAt) {
	...
	if (elapsed_time/1000 < .5) {
		...
		this.DistanciaAcumulada += Math.sqrt( Math.pow(displacement.x , 2) + Math.pow(displacement.y , 2) + Math.pow(displacement.z , 2) );  // Suma cada distancia añadida
		this.VelocidadAcumulada += (this.velocity.x + this.velocity.y + this.velocity.z)/3;													 // Suma cada velocidad añadida
	}

	if (elapsed_time/1000 < .5) {
		var ranking = [48.8461, 47.8461, 2.71748, 5.33999, 98.7529, 77.3556, -88.5938, -39.2653];			// Ranking generado apartir del algoritmo paralelo
													// Lista de condicionales separados por else if para evitar que un ave cumpla multiples condiciones y se evite la
													//   aparicion de aves con colores diversos.
													// Ordenados por orden de ranking alrevez
		if(this.VelocidadAcumulada < ranking[7]){
			this.mesh.material.color = new THREE.Color( 0x02f2d6 );
		}
		else if(this.VelocidadAcumulada < ranking[6]){
			this.mesh.material.color = new THREE.Color( 0x000000 );
		}
		else if(this.VelocidadAcumulada > ranking[5]){
			this.mesh.material.color = new THREE.Color( 0xffff00 );
		}
		else if(this.VelocidadAcumulada > ranking[4]){
			this.mesh.material.color = new THREE.Color( 0x00ff00 );
		}
		else if(this.DistanciaAcumulada < ranking[3]){
			this.mesh.material.color = new THREE.Color( 0x4f1a0f );
		}
		else if(this.DistanciaAcumulada < ranking[2]){
			this.mesh.material.color = new THREE.Color( 0xd500ff );
		}
		else if(this.DistanciaAcumulada > ranking[1]){
			this.mesh.material.color = new THREE.Color( 0xff0000 );
		}
		else if(this.DistanciaAcumulada > ranking[0]){
